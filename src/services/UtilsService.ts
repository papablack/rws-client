...
function mergeDeep<T>(target: T | any, source: T  | any): T 
  {
...
async function fetchSourceMap(jsFilePath: string): Promise<RawSourceMap> 
{
...
async function getCurrentLineNumber(error: Error = null): Promise<number> {
    if(!error){
        error = new Error();
    }    
